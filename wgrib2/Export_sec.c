...
int f_write_sec(ARG2) {
    ...
    if (isdigit(arg1[0]) == 0) fatal_error("export_sec: illegal section %s", arg1);
    n = atoi(arg1);
    if (mode == -1) {
...
int f_read_sec(ARG2) {
    ...
    if (mode == -1) {
        if (isdigit(arg1[0]) == 0) fatal_error("import_sec: illegal section %s", arg1);
	n  = atoi(arg1);
        if (n < 0 || n > 8) fatal_error_i("import_sec: illegal section %d", save->n);
